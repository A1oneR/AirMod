...
public Action afkCheckThread(Handle timer)
{
	...
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && !IsInPause())
		{
			...
			else if (afkKickEnabled)  // if player is on spectators and kick on spectators is enabled ...
			{
				...
				for (int client = 1; client <= MaxClients; client++)
				{
					if (IsClientInGame(client))
					{
						...
						if (!IsPlayerAlive(client) && GetClientTeam(client) == L4D2Team_Survivor && IsFakeClient(client))
						{
							survLimit--;
						}
						if (IsFakeClient(client))
				...
				if ((survReadyCount + infReadyCount) < survLimit + zombLimit) // In game players < ShouldBe
				{
					...
					{
						//PrintHintText(i, "[AFK] 管理员Hint：当前人数%i，补足人数%i，判定需要进场", (survReadyCount + infReadyCount), survLimit + zombLimit);
					}
				} // player is on spectators
				else
				{
					//PrintHintText(i, "[AFK] 管理员Hint：当前人数%i，补足人数%i，判定不需要进场", (survReadyCount + infReadyCount), survLimit + zombLimit);
				}
