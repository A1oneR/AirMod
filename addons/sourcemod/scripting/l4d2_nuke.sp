...
public void OnPluginStart()
{
	...
	g_hNukeTime   = CreateConVar("l4d2_nuke_timer", "720.0", "Set The Time In Seconds At Which Players Will Be Nuked", FCVAR_NOTIFY, true, 1.0, true, 5400.0);
	g_hAnnounce   = CreateConVar("l4d2_nuke_announcer", "300.0", "At What Time In Seconds You Want The Hint Text Announcement To Be Displayed", FCVAR_NOTIFY, true, 0.0, true, 5400.0);
	...
	g_hFinalsOnly = CreateConVar("l4d2_nuke_finals", "0", "Allow Nuke Plugin At Finals Only", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public void Event_LeftStartArea(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_hFinalsOnly.BoolValue && IsFinalMap())
	{
		...
		if(g_hWarning.BoolValue)
		{
			...
			PrintToChatAll("\x04[警告] \x01有在途的核弹打击: \x03%d 分钟 %d 秒", time / 60, time % 60);
		}
	}
...
public Action Cmd_Nuke(int client, int args)
{
	...
	if (g_hFinalsOnly.BoolValue && IsFinalMap())
	{
		if (g_hTimer)
		{
			...
			if(timeleft > 0) PrintToChatAll("\x04[警告] \x01核爆时间所剩: \x03%d 分钟 %d 秒", timeleft / 60, timeleft % 60);
		}
		else
		{
			int timestart = RoundToNearest(g_hNukeTime.FloatValue);
			PrintToChatAll("\x04[警告] \x01核爆时间所剩: \x03%d 分钟 %d 秒", timestart / 60, timestart % 60);
		}
	}
...
public Action Timer_CountDown(Handle timer, int client)
{
	...
	if(time >= 0.0)
	{
		PrintHintTextToAll("核爆所剩时间: %d", RoundToNearest(time));
